...
public class JGraph extends JNativePointer {
    ...
    public JNode GetNodeByIndex(long index) {
        return getNodeByIndex(nativePointer, index);
    }
    // Получить число нод
    ...
    public ArrayList<JNode> GetNodes() {
        ArrayList<long> pointers = getNodes(nativePointer);
    ...
    public JEdge GetEdge(long sourceId, long targetId, boolean allDirections) {
        return getEdge(nativePointer, sourceId, targetId, allDirections);
    } 
    // Получить список ребер у ноды
    public ArrayList<JEdge> GetEdges(long sourceNodeID) {
        ArrayList<long> pointers = getEdges(nativePointer, sourceNodeID);
    ...
    public JNode AddNode(long id, String tag, double latitude, double longitude, int level) {
        return addNodeII(nativePointer, id, tag, latitude, longitude, level);
    }
    ...
    public JNode AddNodeAndEdges(long id, String tag, double latitude, double longitude, int level) {
        return addNodeAndEdges(nativePointer, id, tag, latitude, longitude, level);
    }
    // Добавляем ребро
    public JEdge AddEdge(long sourceId, long targetId) {
        return addEdge(nativePointer, sourceId, targetId);
    }
    ...
    private native ArrayList<long> getNodes(long nativePointer);
    ...
    private native ArrayList<long> getEdges(long nativePointer, long sourceId);
